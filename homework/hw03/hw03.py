...
def taxicab(a, b):
    ...
    9
    """
    a_street, b_street = street(a), street(b)
    a_avenue, b_avenue = avenue(a), avenue(b)
    distance = abs(b_street-a_street)+abs(a_avenue-b_avenue)
    return distance
...
def balanced(m):
    ...
    if is_weight(m):
        ...
        l, r = left(m), right(m)
        left_weight = length(left(m)) * total_weight(end(l))
        right_weight = length(right(m)) * total_weight(end(r))
